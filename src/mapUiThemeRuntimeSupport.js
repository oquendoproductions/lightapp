...
function normalizePublishedMapUiThemeEntry(entry, index = 0) {
  ...
  return {
    id: String(entry?.id || (isDefault ? "default-theme" : `map-ui-theme-${index + 1}`)).trim() || (isDefault ? "default-theme" : `map-ui-theme-${index + 1}`),
...
function resolveMapUiThemeOverride(raw, at = Date.now()) {
  const baseTheme = (() => {
    ...
    return Object.keys(sanitizeMapUiTheme(raw)).length > 0 ? sanitizeMapUiTheme(raw) : {};
  })();
