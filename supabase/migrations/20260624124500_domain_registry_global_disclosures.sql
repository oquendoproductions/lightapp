...
as $$
  select
    ...
    case
      when jsonb_typeof(dd.report_disclosures) = 'array'
        and jsonb_array_length(dd.report_disclosures) > 0
        and jsonb_typeof(tda.report_disclosures) = 'array'
        and jsonb_array_length(tda.report_disclosures) > 0
        then dd.report_disclosures || tda.report_disclosures
      when jsonb_typeof(dd.report_disclosures) = 'array'
        and jsonb_array_length(dd.report_disclosures) > 0
        then dd.report_disclosures
      when jsonb_typeof(tda.report_disclosures) = 'array'
        then tda.report_disclosures
      else '[]'::jsonb
    end as report_disclosures
