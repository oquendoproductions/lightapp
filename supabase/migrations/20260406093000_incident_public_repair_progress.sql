...
alter table public.tenant_domain_configs
  add column if not exists organization_monitored_repairs boolean not null default true;

create table if not exists public.incident_repair_signals (
  ...
  check (length(trim(identity_hash)) > 0),
  unique (tenant_key, domain, incident_id, identity_hash)
);

create index if not exists incident_repair_signals_incident_idx
  on public.incident_repair_signals (tenant_key, domain, incident_id, created_at desc);

create index if not exists incident_repair_signals_identity_idx
  on public.incident_repair_signals (tenant_key, identity_hash, created_at desc);

alter table public.incident_repair_signals enable row level security;

grant select, insert on public.incident_repair_signals to anon, authenticated;
grant usage, select on sequence public.incident_repair_signals_id_seq to anon, authenticated;

drop policy if exists incident_repair_signals_select_tenant on public.incident_repair_signals;
create policy incident_repair_signals_select_tenant
on public.incident_repair_signals
for select
to public
using (
...
drop policy if exists incident_repair_signals_insert_tenant on public.incident_repair_signals;
create policy incident_repair_signals_insert_tenant
on public.incident_repair_signals
for insert
to public
with check (
...
as $$
  ...
  fixed_cutoffs as (
    select
      e.domain,
      e.incident_id,
      max(e.changed_at) filter (where e.new_state in ('fixed'::public.incident_state, 'archived'::public.incident_state)) as last_reset_at,
