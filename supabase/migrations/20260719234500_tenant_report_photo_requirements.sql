alter table public.tenant_domain_assignments
  add column if not exists allow_report_images boolean,
  add column if not exists report_image_required boolean not null default false;

update public.tenant_domain_assignments tda
set allow_report_images = coalesce(dd.allow_report_images, false)
from public.domain_definitions dd
where dd.key = tda.domain_key
  and tda.allow_report_images is null;

update public.tenant_domain_assignments
set allow_report_images = false
where allow_report_images is null;

alter table public.tenant_domain_assignments
  alter column allow_report_images set default false,
  alter column allow_report_images set not null;

alter table public.tenant_domain_assignments
  drop constraint if exists tenant_domain_assignments_report_image_required_check;

alter table public.tenant_domain_assignments
  add constraint tenant_domain_assignments_report_image_required_check
  check (report_image_required = false or allow_report_images = true);
...
as $$
  select
    ...
    dd.report_prefix,
    tda.allow_report_images,
    tda.report_image_required,
    dd.road_required,
    tda.park_required,
