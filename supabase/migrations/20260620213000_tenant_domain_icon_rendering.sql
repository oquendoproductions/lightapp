...
alter table public.tenant_domain_assignments
  add column if not exists icon_render_mode text
    check (icon_render_mode in ('tintable_svg', 'full_color_svg', 'raster')),
  add column if not exists icon_tint_mode text
    check (icon_tint_mode in ('default', 'auto_contrast', 'custom')),
  add column if not exists icon_tint_color text;

update public.tenant_domain_assignments tda
set
  icon_render_mode = case
    when nullif(trim(tda.icon_render_mode), '') is not null then trim(tda.icon_render_mode)
    when coalesce(dd.icon_src, '') ~* '\.svg(?:[?#].*)?$' then 'tintable_svg'
    else 'raster'
  end,
  icon_tint_mode = case
    when nullif(trim(tda.icon_tint_mode), '') is not null then trim(tda.icon_tint_mode)
    else 'auto_contrast'
  end,
  icon_tint_color = nullif(trim(tda.icon_tint_color), '')
