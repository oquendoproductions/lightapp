...
as $$
  ...
  report_rows as (
    ...
    cross join lateral (
      select public.map_incident_domain_from_report(r.light_id, null, r.report_type) as domain
    ) mapped
    where r.tenant_key = public.request_tenant_key()
      ...
      and mapped.domain in (select domain from supported_domains)
  ...
  fixed_cutoffs as (
    ...
    where e.tenant_key = public.request_tenant_key()
      and e.domain in (select domain from supported_domains)
  ...
  repair_signals_ranked as (
    ...
    where s.tenant_key = public.request_tenant_key()
      and s.domain in (select domain from supported_domains)
