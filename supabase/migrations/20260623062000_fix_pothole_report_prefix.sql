...
set report_prefix = 'PH'
where key = 'potholes'
  and coalesce(btrim(report_prefix), '') <> 'PH';
...
begin
  ...
  new.report_number := 'R-PH' || lpad(n::text, 7, '0');
  return new;
end;
$function$;

update public.reports
set report_number = regexp_replace(upper(trim(report_number)), '^R-PHR([0-9]+)$', 'R-PH\1')
where upper(trim(coalesce(report_number, ''))) ~ '^R-PHR[0-9]+$';

update public.pothole_reports
set report_number = regexp_replace(upper(trim(report_number)), '^R-PHR([0-9]+)$', 'R-PH\1')
where upper(trim(coalesce(report_number, ''))) ~ '^R-PHR[0-9]+$';
