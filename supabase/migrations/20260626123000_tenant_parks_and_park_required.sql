...
alter table public.tenant_domain_assignments
  add column if not exists park_required boolean not null default false;

create table if not exists public.tenant_parks (
  id bigint generated always as identity primary key,
  tenant_key text not null references public.tenants(tenant_key) on delete cascade,
  park_key text not null
    check (park_key ~ '^[a-z0-9]+(?:_[a-z0-9]+)*$'),
  park_name text not null,
  boundary_geojson jsonb not null,
  border_color text not null default '#2e7d32',
  border_width numeric(6,2) not null default 2
    check (border_width >= 0 and border_width <= 12),
  fill_color text not null default '#66bb6a',
  fill_opacity numeric(4,2) not null default 0.18
    check (fill_opacity >= 0 and fill_opacity <= 1),
  label_color text not null default '#17314f',
...
drop trigger if exists trg_tenant_parks_updated_at on public.tenant_parks;
create trigger trg_tenant_parks_updated_at
before update on public.tenant_parks
for each row
execute function public.touch_platform_control_plane_updated_at();

create index if not exists idx_tenant_parks_tenant_sort
  on public.tenant_parks(tenant_key, active, sort_order, park_name);

alter table public.tenant_parks enable row level security;
grant select, insert, update, delete on public.tenant_parks to authenticated;

drop policy if exists tenant_parks_select_request_tenant on public.tenant_parks;
create policy tenant_parks_select_request_tenant
on public.tenant_parks
for select
to anon, authenticated
using (tenant_key = public.request_tenant_key());

drop policy if exists tenant_parks_manage_platform_admin on public.tenant_parks;
create policy tenant_parks_manage_platform_admin
on public.tenant_parks
for all
to authenticated
using (
