...
function normalizeUserNotes(raw: unknown): string {
  const text = String(raw || "").trim();
...
async function loadTenantRuntimeConfig(admin: SupabaseClient, tenantKey: string): Promise<TenantRuntimeConfig | null> {
  ...
  const { data, error } = await admin
    .from("tenants")
    .select("tenant_key,boundary_config_key,notification_email_potholes,active")
...
serve(async (req) => {
  ...
  try {
    ...
    const TO_EMAIL = tenantCfg.potholeEmailTo || Deno.env.get("PW_REPORT_TO") || "";
    const FROM_EMAIL =
    ...
    const title = String(body?.title || "").trim() || "Attention Public Works, Pothole report";
    const reportNumber = String(body?.reportNumber || "").trim() || "Unknown";
    const notesRaw = String(body?.notes || "").trim();
    const notes = normalizeUserNotes(notesRaw);
    ...
    const subject = "Attention Public Works, Pothole report";
