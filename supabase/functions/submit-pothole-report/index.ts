...
serve(async (req) => {
  ...
  try {
    ...
    const potholeIdInput = cleanText(body?.pothole_id ?? body?.potholeId, 120);
    const phId = cleanText(body?.ph_id ?? body?.phId, 120);
    ...
    const potholeSelect = "id, tenant_key, ph_id, lat, lng, location_label, nearest_address, nearest_cross_street, nearest_landmark, created_at";
    const reportSelect =
    ...
    let potholeRow: Record<string, unknown> | null = null;

    if (potholeIdInput) {
      const existing = await admin
        .from("potholes")
        .select(potholeSelect)
        .eq("tenant_key", tenantKey)
        .eq("id", potholeIdInput)
        .maybeSingle();
      if (existing.error) {
        return json({ ok: false, error: existing.error.message || "Could not load pothole location" }, 400);
      }
      potholeRow = existing.data as Record<string, unknown> | null;
    }

    if (!potholeRow && phId) {
      const existingByPhId = await admin
        .from("potholes")
        .select(potholeSelect)
        .eq("tenant_key", tenantKey)
        .eq("ph_id", phId)
        ...
        .limit(1)
        .maybeSingle();
      if (!existingByPhId.error && existingByPhId.data) {
        potholeRow = existingByPhId.data as Record<string, unknown>;
      }
    }
    ...
    if (!potholeRow) {
      const created = await admin
        ...
        .single();
      if (created.error) {
        return json({ ok: false, error: created.error.message || "Could not create pothole location" }, 400);
      }
      potholeRow = created.data as Record<string, unknown>;
    }

    const resolvedPotholeId = cleanText(potholeRow?.id, 120);
    if (!resolvedPotholeId) {
      return json({ ok: false, error: "Resolved pothole location is missing an id" }, 500);
    }
    ...
    const existingReport = await admin
      ...
      .select(reportSelect)
      .eq("tenant_key", tenantKey)
      .eq("pothole_id", resolvedPotholeId)
      .eq("lat", lat)
      .eq("lng", lng)
      .eq("note", note)
      .order("created_at", { ascending: false })
    ...
    if (existingReport.error) {
      return json({ ok: false, error: existingReport.error.message || "Could not inspect existing pothole reports" }, 400);
    }
    ...
    if (!reportRow) {
      ...
      if (createdReport.error) {
        return json({ ok: false, error: createdReport.error.message || "Could not create pothole report" }, 400);
      }
